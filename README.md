...
## News
- [2026-09] **TradingAgents v0.4.0** released with a walk-forward backtest runner (`tradingagents backtest`) that scores every call against realized returns, and Portfolio Manager confidence scores with a calibration check in each backtest. See [CHANGELOG.md](CHANGELOG.md) for the full list.
- [2026-09] **TradingAgents v0.3.2** released with data-freshness and robustness fixes: a UTC, look-ahead-safe Yahoo news window, same-day OHLCV cache refresh, a clean CLI error on unusable Windows terminals, and no spurious tool calls from schema-only structured agents.
...
- **Confidence calibration.** The Portfolio Manager states a 0-100 confidence with every rating (shown as `**Confidence**: 72%` in the decision). The backtest compares it with how often the Buy/Sell-side calls were actually right, bucketed by stated confidence, with a Brier score and an over- or underconfidence verdict. If 80% calls come true half the time, the confidence is not worth trusting yet.
